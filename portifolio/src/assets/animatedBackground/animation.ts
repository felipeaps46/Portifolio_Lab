...
function initializeCanvas(canvas: HTMLCanvasElement): [HTMLCanvasElement, CanvasRenderingContext2D] {
...
export function startAnimation(canvas: HTMLCanvasElement) {
  const [c, ctx] = initializeCanvas(canvas);

  for (let i = 0; i < NUM_PARTICLES; i++) {
    particles.push(createParticle(c));
  }

  requestAnimationFrame((time) => draw(time, c, ctx));
}
